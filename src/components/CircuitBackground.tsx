  ...
  return (
    ...
    >
      ...
      <g mask="url(#circuit-radial-mask)">
        {/* Background Grid - Made extremely light and faint to avoid "math notebook" feel */}
        <g stroke={copperColor} strokeWidth="0.5" strokeOpacity="0.035">
          {Array.from({ length: 32 }).map((_, i) => {
            const x = (i + 1) * 60;
            return <line key={`gx-${i}`} x1={x} y1="0" x2={x} y2="1080" />;
          })}
          {Array.from({ length: 18 }).map((_, i) => {
            const y = (i + 1) * 60;
            return <line key={`gy-${i}`} x1="0" y1={y} x2="1920" y2={y} />;
          })}
        </g>
