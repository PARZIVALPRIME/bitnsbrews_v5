...
/**
 * 3. Transistor Electron Carrier Flow Shader
 * Visualizes wave packets of electrons traveling from Source to Drain through the nanosheets.
 */
export const TransistorFlowShader = {
  vertexShader: /* glsl */ `
    varying vec2 vUv;
    varying vec3 vNormal;
    void main() {
  ...
  fragmentShader: /* glsl */ `
    uniform float uTime;
    uniform float uActivity; // 0 (inactive/gated) to 1 (highly conductive)
    uniform vec3 uColor;
    varying vec2 vUv;
    varying vec3 vNormal;

    void main() {
      // Flow coordinate (Source to Drain along U axis)
      float x = vUv.x - uTime * 4.5;
      
      // Generate multiple clean electron wave packets
      float wave = 0.0;
      if (uActivity > 0.02) {
        wave += sin(x * 12.0) * 0.5 + 0.5;
        wave += cos(x * 6.0 + 3.14) * 0.3 + 0.3;
        wave *= uActivity;
      }

      // Base material color (semiconductor silicon base) + glowing electron carriers
      vec3 baseMaterial = vec3(0.08, 0.10, 0.14);
      vec3 activeGlow = uColor * (wave * 2.2);
      vec3 finalColor = baseMaterial + activeGlow;

      // Soft rim catch to maintain structure shape
      float rim = 1.0 - max(0.0, dot(vNormal, vec3(0.0, 0.0, 1.0)));
      finalColor += uColor * pow(rim, 4.0) * 0.35;
